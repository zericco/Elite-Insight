...
namespace RegulatedNoise.Core.EliteInteractions
{
	public class LogFilesScanner : IDisposable
	{
		...
		public LogFilesScanner(string productsPath)
		{
			if (!Directory.Exists(productsPath)) throw new DirectoryNotFoundException("'" + productsPath + "' does not exist");
		...
		{
			string[] netLogs = null;
			string appConfigPath = _productsPath;
		...
		private LocationUpdateEventArgs ParseNetLogFile(Stream netlogFile)
		{
					...
					if (String.IsNullOrEmpty(systemName))
					{
						if (logLump.Contains("System:"))
						{
                            ...
                            _filescanningLog.Log("Stationstring from candidate : " + candidate.Replace("\n", "").Replace("\r", ""));
									...
									GetStation(m, out stationName, out pilotName);
									break;
								}
							}
						}
			...
			return new LocationUpdateEventArgs(systemName, stationName, pilotName);
		}

		private static void GetStation(Match m, out string stationName, out string pilotName)
		{
	...
	public class LocationUpdateEventArgs : EventArgs
	{
		...
		public LocationUpdateEventArgs(string system, string station,string pilot)
		{
			System = system;
			Station = station;
			Pilot = pilot;
