...
namespace RegulatedNoise
{
	class Ocr
	{
		...
		public readonly List<string> ScreenshotBuffer = new List<string>();
		private string CurrentScreenshot;
		public DateTime CurrentScreenshotDateTime;
		private string SystemAtTimeOfScreenshot;
		...
		private EBPixeltest _pixelTest;
		...
		public void PerformOcr(List<Tuple<int, int>> textRowLocations)
		{
			...
			foreach (var row in textRowLocations)
			{
				...
				while (columnCounter < 8)
				{
					...
					int fudgeFactor = 0;// _bOriginal.Height * 6 / 1440;
					...
					if (_callingForm.cbCheckAOne.Checked)
					{
						...
						if (columnCounter == 3)
						{
							...
							_pixelTest.addPicture(brainerousOut, darkPixels);
						}
					}
					else
					{
						...
						{   // It's a text column, we'll use Tesseract
							...
							switch (columnCounter)
							{
								...
								case 3:
									...
									t[result] = Regex.Replace(t[result], @"[a-zA-Z\s]+", string.Empty); // remove any alphas that remain
									break;
								case 5:
								case 7:
			...
			if (_callingForm.cbCheckAOne.Checked)
			{
				_pixelTest.StartModal(_callingForm);
				...
				if (textRowLocations.Count > 0)
				{
					...
					var pr = new Process
					{
						StartInfo =
						{
							...
							FileName = "./Brainerous/nn_training.exe",
							WorkingDirectory = "./Brainerous/"
						}
					};
					pr.Start();
