	...
	public class GameSettings
	{
		...
		void CheckAndRequestVerboseLogging()
		{
			if (AppConfig.Network.VerboseLogging != 1)
			{
				...
				{
					var appconfig = Path.Combine(ApplicationContext.RegulatedNoiseSettings.GamePath, "AppConfig.xml");
		...
		void LoadAppConfig()
		{
			...
			string configFile = Path.Combine(ApplicationContext.RegulatedNoiseSettings.GamePath, "AppConfig.xml");
		...
		void LoadDisplaySettings()
		{
			...
			var configFile = Path.Combine(ApplicationContext.RegulatedNoiseSettings.ProductAppData, "Graphics", "DisplaySettings.xml");
		...
		{
			var path = Path.Combine(ApplicationContext.RegulatedNoiseSettings.ProductAppData, "Graphics");
		...
		{
			_appdataWatcher.Path = ApplicationContext.RegulatedNoiseSettings.GamePath;
