...
namespace RegulatedNoise.TestTab
{
	public partial class TestTab : UserControl
	{
		...
		{
			var matchingSystems = ApplicationContext.Model.StarMap.FindSystem(tbFinderRequest.Text);
			if (matchingSystems == null)
			{
				tbFinderResult.Text = "N/A";
			}
			else
			{
				tbFinderResult.Text = JsonConvert.SerializeObject(matchingSystems);
			}
		}

		private void btFindStation_Click(object sender, EventArgs e)
		{
			var matchingStations = ApplicationContext.Model.StarMap.FindStation(tbFinderRequest.Text);
			if (matchingStations == null)
			{
				tbFinderResult.Text = "N/A";
			}
			else
			{
				tbFinderResult.Text = JsonConvert.SerializeObject(matchingStations);
			}
		}
